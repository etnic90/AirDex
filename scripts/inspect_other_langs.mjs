  ...
  try {
    ...
    for (const id of targetModelIds) {
      const selectRes = await client.query(`
        SELECT model_name, description_es, description_fr 
        FROM public.aircraft_models 
        WHERE id = $1;
      `, [id]);
      
      const { model_name, description_es, description_fr } = selectRes.rows[0];
      
      ['description_es', 'description_fr'].forEach(col => {
        const val = selectRes.rows[0][col];
        if (val) {
          const lines = val.split('\n');
          lines.forEach((line, index) => {
            if (line.includes('#')) {
              console.log(`- Model: "${model_name}" | Col: ${col} | Line ${index + 1}: "${line}"`);
            }
          });
        }
      });
