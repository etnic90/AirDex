...
async function main() {
  try {
    ...
    for (const id of targetModelIds) {
      ...
      keys.forEach(key => {
        const val = row[key];
        if (val && typeof val === 'string' && val.includes('#') && key !== 'id') {
          const lines = val.split('\n');
          lines.forEach((line, idx) => {
            if (line.includes('#')) {
              const isValidHeading = /^###\s+/.test(line) || /^####\s+/.test(line);
              if (!isValidHeading) {
                console.log(`- Model: "${row.model_name}" | Column: "${key}" | Line ${idx+1}: "${line}" [ANOMALY]`);
              }
            }
          });
        } else if (val && typeof val === 'object') {
          // If it's trivia array/json
          const strVal = JSON.stringify(val);
          if (strVal.includes('#')) {
            console.log(`- Model: "${row.model_name}" | Column: "${key}" (JSON) contains '#': ${strVal}`);
          }
        }
      });
    }
