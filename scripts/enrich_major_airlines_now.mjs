...
async function main() {
  try {
    console.log("==========================================");
    console.log("🚀 ENRICHING MAJOR TARGET AIRLINES IMMEDIATELY");
    console.log("==========================================");
    ...
    // Fetch target major airlines
    const majorNames = [
      '%ITA Airways%', '%Aeroitalia%', '%Ryanair%', '%Wizz%', '%Volotea%', 
      '%Air France%', '%Lufthansa%', '%Delta Air%', '%United Airlines%', 
      '%American Airlines%', '%Emirates%', '%Qatar Airways%', '%Singapore Airlines%', 
      '%Turkish Airlines%', '%British Airways%', '%EasyJet%', '%Iberia%'
    ];
    
    console.log("Loading target major airlines...");
    const targetAirlinesRes = await client.query(`
      ...
      WHERE history LIKE '%### Fondazione & Origini%'
        AND (name ILIKE ANY($1))
      ORDER BY name ASC
    `, [majorNames]);

    const targetAirlines = targetAirlinesRes.rows;
    console.log(`Found ${targetAirlines.length} target major airlines to enrich.`);
    ...
    for (let i = 0; i < targetAirlines.length; i++) {
      ...
      console.log(`\n⏳ [${i + 1}/${targetAirlines.length}] Processing Airline: ${fullName} (${iata}/${icao})...`);
      ...
      if (!validWikiTitle) {
        console.log(`  ❌ Wikipedia page not found or validated for "${fullName}"`);
        continue;
      }
    ...
    console.log(`\n🏁 Completed targeted major airlines! Enriched: ${successCount}`);
  } catch (e) {
    console.error(e);
  } finally {
    await client.end();
