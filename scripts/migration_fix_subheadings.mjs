...
async function main() {
  try {
    ...
    for (const id of targetModelIds) {
      ...
      // Matches: optional list characters (* or - or •) followed by spaces, followed by **Specifiche di Bordo:** or **Specifiche di bordo:**
      const updatedDescription = description_it.replace(/^[*\-\s•]*\*\*Specifiche di Bordo:\*\*/gmi, '#### Specifiche di Bordo');

      if (updatedDescription !== description_it) {
        // 3. Update in DB
        ...
        console.log(`- Fixed subheading for ${model_name} (${id}): ${updateRes.rowCount} row(s)`);
      } else {
        console.log(`- Subheading already optimized or not found in ${model_name}.`);
      }
    }

    console.log("\nSubheading migration completed successfully!");
  } catch (err) {
