...
export default function AirlinesClient({ 
  ...
  const stats = useMemo(() => {
    const total = initialAirlines.length;
    const active = initialAirlines.filter(a => !a.closed_year).length;
    const defunct = initialAirlines.filter(a => a.closed_year).length;
  ...
  const filteredAndSortedAirlines = useMemo(() => {
    let result = [...initialAirlines];

    // 1. Ricerca testuale focalizzata solo su Nome, IATA e ICAO
    if (search.trim() !== "") {
    ...
    // 4. Ordinamento (con priorità alla pertinenza della ricerca)
    result.sort((a, b) => {
      if (search.trim() !== "") {
        ...
        const getScore = (item: typeof a) => {
          ...
          // Corrispondenza inizio parola (es. "PRiMA Aero Trasporti Italiani" - "Italiani" inizia con "ita")
          // Usiamo split per evitare crash con caratteri speciali (es. parentesi o punti interrogativi)
          const words = nameLower.split(/[\s\-()]+/);
          if (words.some(word => word.startsWith(q))) return 80;
          
          if (iataLower === q) return 70;
          if (icaoLower === q) return 65;
        ...
        if (scoreA !== scoreB) {
          return scoreB - scoreA; // Decrescente (punteggio maggiore prima)
        }
      }
    ...
    return result;
  }, [initialAirlines, search, statusFilter, allianceFilter, sortBy]);
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10 relative overflow-hidden">
      ...
      <div className="max-w-[1600px] w-[95%] mx-auto relative z-10">
        ...
        {/* Header principale */}
        <div className="mb-10 border-b border-slate-900 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight font-mono leading-none mb-2">
              Terminal Aerolinee
            </h1>
          ...
          <div className="font-mono text-xs text-slate-500 border border-slate-900 bg-slate-950 p-3 rounded-xl flex gap-6">
            ...
            <div>
              ...
              <span className="text-slate-200 font-bold">{initialAirlines.length} Vettori</span>
            </div>
          </div>
        </div>

        {/* CONTATORI TELEMETRIA GENERALI */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 backdrop-blur-sm shadow-sm relative overflow-hidden group hover:border-emerald-500/35 transition-colors">
            ...
            <span className="text-xs font-mono text-slate-400 uppercase tracking-widest block mb-1.5 font-bold">Flotte Attive</span>
            <span className="text-3xl font-black text-emerald-400 font-mono block">{stats.active}</span>
            <span className="text-xs text-emerald-500/70 font-mono block mt-1">{stats.total > 0 ? Math.round((stats.active/stats.total)*100) : 0}% del totale operanti</span>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 backdrop-blur-sm shadow-sm relative overflow-hidden group hover:border-emerald-500/35 transition-colors">
            ...
            <span className="text-xs text-amber-600/70 font-mono block mt-1">{stats.defunct} aerolinee confluite o chiuse</span>
          </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 mb-8 backdrop-blur-md relative overflow-hidden shadow-inner">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
            ...
            <div className="lg:col-span-3">
              ...
              <div className="grid grid-cols-3 bg-slate-950/80 p-1 rounded-2xl border border-slate-900 font-mono text-xs">
                <button 
                  ...
                  className={`py-2 rounded-xl transition-all cursor-pointer ${statusFilter === "all" ? "bg-slate-900 text-white font-bold" : "text-slate-500 hover:text-slate-350"}`}
                >
                  Tutti
                </button>
                <button 
                  onClick={() => setStatusFilter("active")}
                  className={`py-2 rounded-xl transition-all cursor-pointer ${statusFilter === "active" ? "bg-emerald-500/10 text-emerald-400 font-bold border border-emerald-500/20" : "text-slate-500 hover:text-slate-350"}`}
                >
                  Attivi
                </button>
                <button 
                  onClick={() => setStatusFilter("defunct")}
                  className={`py-2 rounded-xl transition-all cursor-pointer ${statusFilter === "defunct" ? "bg-amber-500/10 text-amber-500 font-bold border border-amber-500/20" : "text-slate-500 hover:text-slate-355"}`}
                >
                  Storici
                </button>
              </div>
            ...
            <div className="lg:col-span-2">
              ...
              <select 
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value as "name" | "founded_asc" | "founded_desc" | "country")}
                className="w-full p-3 rounded-2xl bg-slate-950/80 text-slate-300 border border-slate-900 focus:border-emerald-500/80 focus:outline-none font-mono text-xs transition-all cursor-pointer"
              >
                ...
                >
                  ...
                  <div className="space-y-3 font-mono text-xs text-slate-400 mb-6 border-t border-slate-900/40 pt-5 relative z-10">
                    <div className="flex justify-between items-center gap-2">
                      <span className="text-slate-550 uppercase text-xs font-bold shrink-0">Hub Principale:</span>
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-slate-550 uppercase text-xs font-bold">Alleanza:</span>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 border-t border-slate-900 pt-6 mb-10">
            ...
            <div className="flex items-center gap-1.5 font-mono text-xs">
              ...
              <button
                ...
                className="p-2.5 rounded-xl border border-slate-900 bg-slate-950 text-slate-400 hover:text-white disabled:opacity-30 disabled:pointer-events-none transition-colors cursor-pointer"
                title="Prima Pagina"
              >
              ...
              <button
                ...
                disabled={currentPage === 1}
                className="px-3.5 py-2.5 rounded-xl border border-slate-900 bg-slate-950 text-slate-400 hover:text-white disabled:opacity-30 disabled:pointer-events-none transition-colors flex items-center gap-1 cursor-pointer"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                .map((pageIdx, index, arr) => {
                  ...
                  items.push(
                    <button
                      ...
                      className={`px-3.5 py-2.5 rounded-xl transition-all border cursor-pointer ${
                        currentPage === pageIdx
