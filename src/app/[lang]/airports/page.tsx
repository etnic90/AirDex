...
export default function AirportsPage() {
  ...
  const stats = useMemo(() => {
    if (airports.length === 0) return { total: 0, countries: 0, avgElevation: 0 };
    const countries = new Set(airports.map(a => a.country).filter(Boolean));
    const totalElev = airports.reduce((acc, a) => acc + (a.elevation_ft || 0), 0);
    return {
      total: airports.length,
      countries: countries.size,
      avgElevation: Math.round(totalElev / airports.length)
    };
  }, [airports]);

  // Elenco unico delle nazioni caricate per il filtro dropdown
  const countriesList = useMemo(() => {
    const list = Array.from(new Set(airports.map(a => a.country).filter(Boolean)));
    return list.sort();
  }, [airports]);

  // Logica di filtraggio e ordinamento
  const filteredAndSortedAirports = useMemo(() => {
    let result = airports.filter(a => {
      const matchesSearch = 
  ...
  }, [airports, search, selectedCountry, runwayFilter, sortBy]);

  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 text-emerald-500 flex flex-col items-center justify-center font-mono gap-4 w-full">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 font-sans">
          ...
          <div className="bg-slate-900/90 p-8 rounded-3xl border border-slate-800 border-b-4 border-b-cyan-500 shadow-lg hover:-translate-y-1 transition-all duration-300">
            <span className="text-xs text-slate-400 uppercase font-black tracking-widest block mb-2 font-mono">Nazioni Raggiunte</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div className="md:col-span-2">
              ...
              <input 
                ...
                className="w-full p-4 rounded-xl bg-slate-950 text-white border border-slate-850 focus:border-emerald-500 focus:outline-none text-sm transition-all shadow-[inset_0_0_10px_rgba(0,0,0,0.5)] placeholder:text-slate-655"
              />
            </div>
                  ...
                  >
                    ...
                    <div className="h-40 w-full bg-slate-950 relative overflow-hidden flex-shrink-0">
                      {airport.image_url && airport.image_url !== 'NOT_FOUND' ? (
                        ...
                        <div className="w-full h-full bg-gradient-to-br from-slate-900/40 to-slate-950/60 flex items-center justify-center border-b border-slate-850">
                          <span className="text-xs text-slate-550 tracking-widest font-mono uppercase font-black">Radar No Image</span>
                        </div>
                      )}
